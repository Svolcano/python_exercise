...
def count_10010_sid_hit(iris):
    ax1 = plt.subplot(1,1,1,facecolor='white')
    ax1.set_title('7 day sid hit')            #设置图体，plt.title
    ax1.set_xlabel('sid hit count')                    #设置x轴名称,plt.xlabel
    ax1.set_ylabel('date ')                    #设置y轴名称,plt.ylabel
    ax1.plot(iris['count_10010_sid_hit'],iris['rpt_date'],alpha=0.5,color='r')   #线图：linestyle线性，alpha透明度，color颜色，label图>例文本
...
if __name__=='__main__':
    ...
    #print rpt
    #rpt=fusion_data_all
    col_10010=[x for x in rpt.columns if '10010' in x]
    col_10086=[x for x in rpt.columns if '10086' in x]
    col_189=[x for x in rpt.columns if '189' in x]
